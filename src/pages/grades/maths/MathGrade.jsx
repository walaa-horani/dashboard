...
function MathGrade() {
  ...
  useEffect(() => {
    ...
    fetch('https://walaadashboard.pythonanywhere.com/api/mathGrade/')
  ...
  const handleDelete = () => {
    if (idToDelete) {
      axios.delete(`https://walaadashboard.pythonanywhere.com/api/mathGrade/${idToDelete}`)
        .then(res => {
  ...
  <td>{his.student}</td>
          <td>{his.teacher}</td>
           ...
           <td style={{width:'50px'}}> <Link to={`/EditMathGradeDetails/${his.id}`}><Icon style={{fontSize:'24px'}} icon="openmoji:edit" /></Link></td>
