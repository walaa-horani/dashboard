...
function OutDoors() {
  ...
  useEffect(() => {
    ...
    fetch('https://walaadashboard.pythonanywhere.com/api/outdoor/')
      .then((response) => response.json())
      .then((data) => setOutDoors(data))
      .catch((error) => console.error('Error fetching data:', error));
  }, []);
  const handleDelete = () => {
    if (idToDelete) {
      axios.delete(`https://walaadashboard.pythonanywhere.com/api/outdoor/${idToDelete}`)
        .then(res => {
          // If the deletion is successful, update the state to trigger a re-render
          setOutDoors(prevoutdoorss => prevoutdoorss.filter(outdoors => outdoors.id !== idToDelete));
          // Close the dialog
          handleClose();
          // Navigate to '/'
          navigate('/outdoors');
  ...
  <td >{outdoors.student} </td>
            ...
            <td>{outdoors.time}</td>
            <td>{outdoors.date}</td>
            <td>{outdoors.place}</td>
           
            
           <td style={{width:'50px'}}> <Link to={`/Editoutdoors/${outdoors.id}`}><Icon style={{fontSize:'24px'}} icon="openmoji:edit" /></Link></td>
           <td  style={{ fontSize:'24px',width:'50px'}} > <button onClick={() => handleClickOpen(outdoors.id)} ><Icon   icon="flat-color-icons:delete-row" /></button></td>
