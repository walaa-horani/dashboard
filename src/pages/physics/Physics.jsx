...
function Physics() {
  ...
  useEffect(() => {
    ...
    fetch('https://walaadashboard.pythonanywhere.com/api/physics/')
  ...
  const handleDelete = () => {
    if (idToDelete) {
      axios.delete(`https://walaadashboard.pythonanywhere.com/api/physics/${idToDelete}`)
        .then(res => {
          // If the deletion is successful, update the state to trigger a re-render
          setPhysics(prevphysicss => prevphysicss.filter(physics => physics.id !== idToDelete));
          // Close the dialog
          handleClose();
          // Navigate to '/'
          navigate('/physics');
  ...
  <tbody>
        {physics.map((physics) => (
          <tr className='text-center' key={physics.id}>
           ...
           <td style={{width:'50px'}}> <Link to={`/Editphysics/${physics.id}`}><Icon style={{fontSize:'24px'}} icon="openmoji:edit" /></Link></td>
           <td  style={{ fontSize:'24px',width:'50px'}} > <button onClick={() => handleClickOpen(physics.id)} ><Icon   icon="flat-color-icons:delete-row" /></button></td>
